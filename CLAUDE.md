...
- **Every agent logs events** (task_start/task_end/handoff_sent/blocked...) as JSONL lines appended to `.coordination/metrics/activity.jsonl`. This feeds `/team-metrics` and `/team-office` (a dependency-free Node SSE server + canvas office in `templates/team-office/` that renders agents at desks with live status and animated handoffs).
