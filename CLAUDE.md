...
- **Nested agent delegation is blocked.** Worker agents carry `disallowedTools: Agent` in frontmatter, and a `PreToolUse` hook (`hooks/block-nested-delegation.py`) denies Agent/Task calls made from inside dev-team subagents — this prevents token waste like backend spawning nested backends. Allowed exceptions: `qa` → `qa-frontend`/`qa-backend`, `lead` → any team agent except another `lead`, anyone → `Explore`. Main-session delegation is untouched.
