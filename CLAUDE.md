...
### dev-team
A **team of 20 role-agents** covering the full SDLC that collaborate to build/operate real projects (mono-repo or multi-repo). This is the substantial plugin; understand its model before editing it.

**Agents** (`plugins/dev-team/agents/*.md`): `setup`, `product-owner`, `architect`, `ui-designer` (Design Lead), `ux-researcher`, `visual-designer`, `motion-designer`, `artist-3d`, `design-engineer`, `lead`, `backend`, `frontend`, `dba`, `qa` (QA Lead), `qa-frontend`, `qa-backend`, `release-manager`, `infra`, `cybersec`, `tech-writer`. Each is a Markdown file with frontmatter (`name`, `description`, `model`, `tools`). The `description` is what Claude Code uses to decide when to auto-delegate, so it matters functionally. The `model` field is deliberately tiered to optimize token cost: `haiku` for setup/tech-writer, `sonnet` for everyone else (architect included — it defaults to sonnet, upgradeable via `team.models.architect: "opus"` in `.coordination/config.json` (per project) or `~/.claude/dev-team.config.json` (per user, project config wins), which the lead/flows pass as a model override when invoking; haiku-default agents (setup, tech-writer) are FIXED — overrides for them are ignored). Keep that discipline when adding agents, and never use `fable`.

**Commands** (`plugins/dev-team/commands/*.md`): 24 slash commands (`/bug` enforces reproduce-before-register: qa reproduces with evidence and emits REPRODUCIDO / NO REPRODUCIDO / BLOQUEADO before the PO may create the bug or the lead may triage; `/design` runs the design team: brief → UX + visual in parallel → 2-3 screen directions → user picks → motion + 3D → design-engineer prototype/tokens → deck). `/start` is the universal entry point (detects context, routes the user — simplicity is a core design goal). Others: `/new-project`, `/onboard`, `/setup`, `/refine`, `/assign-task`, `/inbox`, `/handoff`, `/sync`, `/test-plan`, `/e2e`, `/review-pr`, `/git-check`, `/security-audit`, `/db-health`, `/deploy-check`, `/document`, `/status`, `/pase` (environment release requests via release-manager), `/team-metrics` (per-agent productivity + token usage, for the Lead), `/wiki` (init/ingest/lint/query of the project wiki, run by tech-writer), `/team-office` (live 2D virtual-office dashboard). Frontmatter `description` + `argument-hint`; body is the procedure, with `$ARGUMENTS` interpolated. Commands orchestrate; agents do the work.

**Templates** (`plugins/dev-team/templates/`): scaffolding the `/new-project` flow copies when creating repos — `dotnet-microservice`, `nodejs-microservice`, `react-spa`, `react-microfrontend`, gateway variants (`gateway-yarp`, `gateway-ocelot`, `gateway-traefik`), `project-umbrella`, and `e2e-faast` (the QA suite: Playwright config with evidence routed to `.coordination/evidence/`, axe fixture, auth/data fixtures, Schemathesis script, E2E workflow; `{{ProjectName}}` placeholder). Filenames/contents use `{{ServiceName}}` placeholders. `templates/pase/` holds the release-request docx template (`Plantilla_Solicitud_Pase_Devs.docx`) that release-manager fills.
...
- **The design team is a gate for new screens.** `ui-designer` is the Design Lead (same pattern as `qa`): it splits work by handoff to `ux-researcher`, `visual-designer`, `motion-designer`, `artist-3d`, `design-engineer`, and consolidates a *propuesta funcional de diseño* under `.coordination/design/DSN-nnn-*/` (brief, UX, 2-3 directions, visual, motion, 3D, navigable prototype + W3C DTCG `tokens.json` + `DESIGN.md`, deck `propuesta.html/.pdf`). Self-contained HTML is the source of truth; Pencil/Figma/Penpot MCPs (configured in `design.tools` by `setup design`) are sync mirrors. Designers never commit app code; frontend implements only from the approved handoff (lead gate 6). Plugin `skills/` ship design foundations (design, motion, a11y, DTCG tokens, web 3D) so the team works without the user's personal skills, but uses them when present.
